...
void Enemy::chooseEnemy(int idNumber) {
	...
	switch(idNumber) {
		case 0: enemy.setAttributes(100, 100, 15, 0, 6, "Bed", "A messy bed that looks like it's just been slept on.", "Ouch...", &gBedBattleTexture); break;
	}
}

int Enemy::attack(BattleChara* battleLouis) {
	if(!takingCover)
		return mAttack - battleLouis->getDef();
	else
		return (mAttack - battleLouis->getDef())/2;
}
